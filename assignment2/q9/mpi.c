...
int main(int argc, char *argv[]) {
  ...
  for (int run = 0; run < N_RUNS; run++) {
    ...
    if (!rank) {
      ...
      MPI_Send(&data, DATA_COUNT, MPI_INT, 1, 0, MPI_COMM_WORLD);
      MPI_Recv(&data, DATA_COUNT, MPI_INT, 1, 0, MPI_COMM_WORLD, &status);
    }

    if (rank) {
      MPI_Recv(&data, DATA_COUNT, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
      MPI_Send(&data, DATA_COUNT, MPI_INT, 0, 0, MPI_COMM_WORLD);
    }
    ...
    total_time_no_init +=
        (end_no_init - start_no_init - timer_overhead_no_init);
    free(data);
  }
  ...
  printf("Processes=%d Arraysize=%d AvgTimeInit=%.6f AvgTimeNoInit=%.6f sec\n",
